ifndef::backend-pdf[]
[notitle]
endif::[]
= Modelldokumentation Title
ifdef::env-github,backend-pdf[]
:title-page:
:title-logo-image: image:./docs/img/header_sz_pdf.jpg[top=-1%,align=center]
//:title-page-background-image: image:./docs/img/header_sz_pdf.pdf[]
endif::[]
//ifdef::backend-html5,backend-pdf[]
ifndef::env-github[:toc: left]
//:title: Modelldokumentationen
:toc-title: Inhalt
:toclevels: 3
:figure-caption: Abbildung
:example-caption!:
:table-caption: Tabelle
:note-caption: Hinweis
// zum Generieren eines PDF aus VS Code (HTML muss mit eingebetteten Bildern generiert werden, damit daraus ein korrektes PDF erstellt werden kann)
:data-uri:
:source-highlighter: highlight.js
:sectnums:
:sectnumlevels: 5
:sectlinks:
:sectanchors:
:includedir: docs
:stylesheet: themes/stylesheet.css
:pdf-theme: themes/pdf-theme.yml
:pdf-fontsdir: fonts/
:doctype: book

// Todo Header Styling und Environment ändern auf Doc. Header auf der Webseite muss auf andere Weise integriert werden

ifdef::backend-pdf[]
//include::{includedir}/headerSZ.adoc[]
= <Titel der Doku pdf>
endif::[]

ifndef::env-github,backend-pdf[]
[.img-logo]
image:{includedir}/img/logo_kt_sz_rgb.jpg[]


= <Title der Doku>
[.subtitle]#<Untertitel der Modelldokumentation># +

****

* https://ch-sz-geo.github.io/A000_entrypage/[zur Startseite] +
* https://github.com/AmandaWetter/A900/tree/main/data[Übersicht Modelle] +
* https://ch-sz-geo.github.io/A000/ebook.pdf[Dokumentation als Pdf] +

****
endif::[]

// Die Verwiese auf die Inhalte der Dokumentation sollen im README auf Github nicht angezeigt werden.
ifndef::env-github[]
include::{includedir}/impressum.adoc[]
include::{includedir}/allgemeines.adoc[]
include::{includedir}/modellbeschreibung.adoc[]
include::{includedir}/diagramme.adoc[]
include::{includedir}/klassenbeschreibung.adoc[]
endif::[]

// Erklärung und Link auf Website
ifdef::env-github[]
[discrete]
== Modelldokumentation
Die ganze Übersicht der Modelldokumentation befindet sich unter folgendem Link:
https://ch-sz-geo.github.io/A000_entrypage/[Modelldokumentation Startseite]
endif::[]